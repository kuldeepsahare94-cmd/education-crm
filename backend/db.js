...
if (roleCount === 0) {
  const modules = ['leads', 'students', 'courses', 'admissions', 'payments', 'companies', 'placements', 'reports', 'users', 'settings', 'assistant', 'whatsapp', 'lead_sources', 'attendance'];
  ...
  const roleDefaults = {
    ...
    'Counselor': {
      ...
      assistant: [1, 1, 0, 0, 0], whatsapp: [0, 0, 0, 0, 0], lead_sources: [0, 0, 0, 0, 0], attendance: [1, 1, 1, 0, 1],
    },
    'Accountant': {
      leads: [0, 0, 0, 0, 0], students: [1, 0, 0, 0, 0], courses: [1, 0, 0, 0, 0],
      admissions: [1, 0, 0, 0, 0], payments: [1, 1, 1, 0, 1], companies: [0, 0, 0, 0, 0],
      placements: [0, 0, 0, 0, 0], reports: [1, 0, 0, 0, 1], users: [0, 0, 0, 0, 0], settings: [0, 0, 0, 0, 0],
      assistant: [1, 1, 0, 0, 0], whatsapp: [0, 0, 0, 0, 0], lead_sources: [0, 0, 0, 0, 0], attendance: [0, 0, 0, 0, 0],
    },
    'Placement Officer': {
      leads: [0, 0, 0, 0, 0], students: [1, 0, 0, 0, 0], courses: [1, 0, 0, 0, 0],
      ...
      assistant: [1, 1, 0, 0, 0], whatsapp: [1, 0, 0, 0, 0], lead_sources: [0, 0, 0, 0, 0], attendance: [0, 0, 0, 0, 0],
    },
    'Trainer': {
      leads: [0, 0, 0, 0, 0], students: [1, 0, 0, 0, 0], courses: [1, 0, 0, 0, 0],
      ...
      assistant: [1, 0, 0, 0, 0], whatsapp: [0, 0, 0, 0, 0], lead_sources: [0, 0, 0, 0, 0], attendance: [1, 1, 1, 0, 1],
    },
  };
...
CREATE TABLE IF NOT EXISTS batches (
  ...
  course_id INTEGER,
  trainer TEXT,
  schedule_text TEXT,          -- free text, e.g. "Mon-Fri, 9:00-11:00 AM"
  start_date TEXT,
  end_date TEXT,
  status TEXT DEFAULT 'Active', -- Upcoming | Active | Completed | Cancelled
  created_by INTEGER,
  created_at TEXT DEFAULT (datetime('now')),
  FOREIGN KEY (course_id) REFERENCES courses(id) ON DELETE SET NULL,
  FOREIGN KEY (created_by) REFERENCES users(id) ON DELETE SET NULL
...
-- A batch's roster. Kept as an explicit join table (rather than reusing
-- admissions.batch, which was just free text) so a student can be added or
-- removed from a batch's attendance list independent of their admission record.
...
-- One row per batch per day attendance is taken.
...
-- Per-student outcome for a session. New sessions default every enrolled
-- student to Present — the "auto-marks present, flip the absentees" behavior.
CREATE TABLE IF NOT EXISTS attendance_records (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  session_id INTEGER NOT NULL,
  student_id INTEGER NOT NULL,
  status TEXT DEFAULT 'Present',  -- Present | Absent | Late | Leave
  marked_via TEXT DEFAULT 'manual', -- manual | biometric | import
  check_in_time TEXT,
  remarks TEXT,
  created_at TEXT DEFAULT (datetime('now')),
  updated_at TEXT DEFAULT (datetime('now')),
...
// Biometric device enrollment ID, for matching punch-import records to a student.
ensureColumn2('students', 'biometric_id', 'biometric_id TEXT');
